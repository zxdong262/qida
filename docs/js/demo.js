/**
 * demo.js for ponder code standard
...
$(function($) {
	...
	//pre javascript
	$('pre.html').escape()
	$('pre').wrapInner('<code contentEditable="true" />')
	$('#wrapper > h2').addClass('section-title')
	$('#wrapper > h3').addClass('sub-title')
	...
	res.each(function(index) {
		...
		,next = res.eq(index + 1)
		,navstr = '#nav-' + index
		tops.push(t.offset().top + 3 * index)
	...
	$('#wrapper').append('<div id="nav"><ul class="nav-ul">' + ht + '</ul></div><a class="bg-fff label border corner nav-open" id="nav-toggle" href="javascript:;"><span class="open-txt">Hide &raquo;</span><span class="close-txt">&laquo; Show</span></a>')
	highlightNav()
	setNavHeight()
